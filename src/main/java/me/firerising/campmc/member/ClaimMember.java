...
public class ClaimMember {
    ...
    public void eject(Location location) {
        ...
        Location spawn = Main.getInstance().getPluginSettings().getSpawnPoint();
        if (spawn == null && location == null) return;
        Bukkit.getScheduler().runTask(Main.getInstance(), () -> player.getPlayer().teleport(location == null ? spawn : location));
